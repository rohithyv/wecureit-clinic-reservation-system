...
/** True if doctor may practice this specialty in this state (per licenseBySpecialty). */
export function doctorLicensedForSpecialtyInState(doctor, specialtyId, state) {
  if (!specialtyId || !state) return true
  const m = doctor?.licenseBySpecialty
  if (m && typeof m === 'object' && Array.isArray(m[specialtyId]) && m[specialtyId].length > 0) {
    return m[specialtyId].includes(state)
  }
  if (doctor?.licensedStates?.length) return doctor.licensedStates.includes(state)
  return false
}

/** Doctor has at least one specialty licensed in the given state. */
export function doctorHasAnyLicenseInState(doctor, state) {
  if (!state) return true
  const m = doctor?.licenseBySpecialty
  if (m && typeof m === 'object' && Object.keys(m).length > 0) {
    return Object.values(m).some((arr) => Array.isArray(arr) && arr.includes(state))
  }
  if (doctor?.licensedStates?.length) return doctor.licensedStates.includes(state)
  return false
}

export function formatDoctorLicenseSummary(doctor) {
  if (!doctor?.licenseBySpecialty || typeof doctor.licenseBySpecialty !== 'object') {
    return doctor?.licensedStates?.join(', ') || '—'
  }
  const parts = Object.entries(doctor.licenseBySpecialty).map(([specId, states]) => {
    const name = SPECIALIZATIONS.find((s) => s.id === specId)?.name || specId
    const st = Array.isArray(states) ? states.join(', ') : ''
    return `${name}: ${st}`
  })
