...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.error) {
      ...
      navigate('/login', { state: { registered: true, roleHint: formData.role || 'patient' } })
    }
  }

  return (
    <div className="min-h-screen bg-rose-950 flex items-center justify-center py-12 px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl flex flex-col relative z-10">
        ...
        <div className="bg-white rounded-[48px] shadow-[0_40px_80px_-15px_rgba(0,0,0,0.3)] flex flex-col lg:flex-row overflow-hidden border border-white/20">          
          ...
          <div className="lg:w-[40%] bg-rose-900 p-12 text-white relative overflow-hidden flex flex-col justify-between">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_bottom_left,_var(--tw-gradient-stops))] from-rose-400/25 via-rose-900 to-rose-950" />
            ...
            <div className="relative z-10">
              ...
              <p className="text-rose-50 text-sm leading-relaxed mb-10 opacity-80">
              ...
              <ul className="space-y-4">
                {['Direct Doctor Access', 'Real-time Booking', 'Secure Medical Records'].map((item) => (
                  <li key={item} className="flex items-center gap-3 text-xs font-medium text-rose-50">
                    <CheckCircle2 size={16} className="text-rose-300" /> {item}
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="relative z-10 pt-12 border-t border-white/10">
              <p className="text-[10px] uppercase tracking-widest font-bold opacity-50 text-rose-100">Trusted by 10k+ Patients</p>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  ...
                  <input name="firstName" type="text" placeholder="John" onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-rose-500/10 focus:border-rose-700 transition-all" required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Last Name</label>
                  <input name="lastName" type="text" placeholder="Doe" onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-rose-500/10 focus:border-rose-700 transition-all" required />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  ...
                  <input name="email" type="email" placeholder="john@example.com" onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-rose-500/10 focus:border-rose-700 transition-all" required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Phone</label>
                  <input name="phone" type="tel" placeholder="+1 555-0123" onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-rose-500/10 focus:border-rose-700 transition-all" required />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input name="password" type={visible ? "text" : "password"} placeholder="••••••••" onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-rose-500/10 focus:border-rose-700 transition-all" required />
              ...
              <div className="pt-6 border-t border-slate-100">
                ...
                <div className="bg-rose-50/50 rounded-2xl p-4 mb-4 flex gap-3 border border-rose-100/50">
              ...
              <div className="flex flex-col gap-4 pt-4">
                <button type="submit" className="w-full bg-rose-700 hover:bg-rose-800 text-white font-bold py-4 rounded-2xl shadow-xl shadow-rose-900/25 transition-all active:scale-[0.98]">
                  Register Account
                </button>
                <p className="text-center text-slate-500 text-sm">
                  Already have an account? <Link to="/login" className="text-rose-700 font-bold hover:underline">Sign in</Link>
                </p>
              </div>
            </form>
          </div>
