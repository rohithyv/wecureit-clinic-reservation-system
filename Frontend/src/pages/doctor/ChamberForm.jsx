...
export default function ChamberForm() {
  ...
  const [hiddenBilling, setHiddenBilling] = useState({
  ...
  useEffect(() => {
    if (existing) {
      ...
      setHiddenBilling({
        ...
        consultationCharge: existing.consultationCharge ?? 100,
      })
    }
  }, [existing])
  ...
  const handleSubmit = (e) => {
    ...
    if (tooShort) {
      setError(`Shift must be at least ${MIN_DOCTOR_HOURS_PER_DAY} hours per day.`)
      return
    }
    if (tooLong) {
      setError(`Doctors can work at most ${MAX_DOCTOR_HOURS_PER_DAY} hours per day at one facility.`)
      return
    }
    if (!doctor) return
    ...
    const payload = {
      ...
      clinicId: fid,
      doctorId: doctor.id,
      slotDurationMinutes: hiddenBilling.slotDurationMinutes,
      consultationCharge: hiddenBilling.consultationCharge,
    }
    if (isEdit) {
  ...
  return (
    <PageShell
      ...
      subtitle="Choose facility and room, then set your shift window and weekdays. Session length and pricing are configured separately."
      heroSize="compact"
      ...
      <form onSubmit={handleSubmit} className="card p-8 md:p-10 space-y-8 rounded-[2rem]">
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setForm((f) => ({ ...f, facilityId: '', clinicId: '' }))
            }}
        ...
        <div className="flex gap-3">
          <button type="submit" className="btn-primary">
            {isEdit ? 'Save changes' : 'Add availability'}
          </button>
          <Link to="/doctor/chambers" className="btn-secondary">
            Cancel
          </Link>
        </div>
      </form>
