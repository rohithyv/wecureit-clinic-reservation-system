...
export default function BookAppointment() {
  ...
  const [cardSavedBanner, setCardSavedBanner] = useState(false)
  ...
  const room = useMemo(() => getRoomForChamber(chamber), [chamber])
  ...
  const handleBook = async () => {
    ...
    const displayName = [user.firstName, user.lastName].filter(Boolean).join(' ') || user.name || 'Patient'
    const aptData = {
      patientId: user.id,
      ...
      chamberName: facility?.name ? `${facility.name}${room?.code ? ` · ${room.code}` : ''}` : 'Medical Facility',
  ...
  return (
    ...
    >
      ...
      {cardSavedBanner && (
        <div className="mb-8 p-4 rounded-2xl bg-rose-50 text-rose-800 border border-rose-100 text-center text-xs font-black uppercase tracking-[0.15em] animate-in fade-in">
          Credit card information has been saved to your profile.
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-12 gap-12">
        <div className="lg:col-span-7 space-y-12">
          ...
          <section>
            ...
            <div className="flex gap-2 bg-slate-100/50 p-1.5 rounded-2xl max-w-sm mx-auto">
              {(APPOINTMENT_DURATIONS || [15, 30, 60]).map((mins) => (
                <button
                  key={mins}
                  type="button"
                  onClick={() => {
          ...
          {selectedDate ? (
            <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 text-center">
                3. Available Slots for {new Date(selectedDate + 'T12:00:00').toLocaleDateString('en-US', { month: 'long', day: 'numeric' })} ({durationMinutes}m)
              </h2>
          ...
          <section className="pt-10 border-t border-slate-100 grid md:grid-cols-1 gap-10">
            <div>
            ...
            <div className="space-y-6 mb-10">
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  ...
                  {room?.code && (
                    <p className="text-[10px] font-black text-rose-600 uppercase tracking-widest mt-1">Room {room.code}</p>
                  )}
                </div>
              </div>
